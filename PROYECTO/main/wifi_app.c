...
{	
    ...
    if (retry < retry_count)
    {
        ...
        ESP_LOGI(TAG, "System time is set! Hora local (Colombia): %s", time_str);
...
void save_ap_credentials(const char *ssid, const char *password) {
    ...
    ESP_ERROR_CHECK(nvs_set_str(nvs_handle, "ap_password", password));
    ESP_ERROR_CHECK(nvs_commit(nvs_handle));
    nvs_close(nvs_handle);
}

static void load_ap_credentials(char *ssid, size_t ssid_len, char *password, size_t password_len) {
    nvs_handle_t nvs_handle;
    size_t required_size;

    strlcpy(ssid, WIFI_AP_SSID, ssid_len);
    strlcpy(password, WIFI_AP_PASSWORD, password_len);
    ...
    if (nvs_get_str(nvs_handle, "ap_ssid", ssid, &required_size) != ESP_OK) {
        strlcpy(ssid, WIFI_AP_SSID, ssid_len);
    }

    required_size = password_len;
    if (nvs_get_str(nvs_handle, "ap_password", password, &required_size) != ESP_OK) {
        strlcpy(password, WIFI_AP_PASSWORD, password_len);
    }
...
esp_err_t wifi_app_set_ap_credentials(const char *ssid, const char *password) {
    wifi_config_t ap_config = {0};

    if (ssid == NULL || password == NULL || strlen(ssid) == 0 || strlen(ssid) > MAX_SSID_LENGTH) {
    ...
    save_ap_credentials(ssid, password);
    return ESP_OK;
}
	...
	else if (event_base == IP_EVENT)
	{
		switch (event_id)
		{
			case IP_EVENT_STA_GOT_IP:
			{
				...
				ESP_LOGI(TAG, "IP_EVENT_STA_GOT_IP -> IP del ESP32 en tu red: " IPSTR,
...
{
	char ap_ssid[MAX_SSID_LENGTH + 1];
	char ap_password[MAX_PASSWORD_LENGTH + 1];
	load_ap_credentials(ap_ssid, sizeof(ap_ssid), ap_password, sizeof(ap_password));
