...
static esp_err_t read_current_potentiometer_for_uart(int *out_pot_raw, uint8_t *out_pwm_value)
{
    ...
    *out_pot_raw = pot_raw;
    *out_pwm_value = adc_raw_to_pwm_8bit(pot_raw);
    return ESP_OK;
}
...
static void rgb2_state_machine_task(void *arg)
{
    ...
    while (true) {
        ...
        set_rgb_pwm(&LED_RGB2, visible_red, visible_green, visible_blue);
...
void app_main(void)
{
    ...
    const uart_usage_callbacks_t uart_callbacks = {
        ...
        .read_potentiometer = read_current_potentiometer_for_uart,
    };
    ESP_ERROR_CHECK(uart_usage_init(&uart_callbacks));

    // Verificacion solicitada: imprimir lecturas crudas al arrancar.
